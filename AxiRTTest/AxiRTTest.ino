...
void setup() 
{
  ...
  pauseButton = new SimpleButton("Pause Test1", xbutton, MousePaperBottom + 20, font_CB, 20, TextColor, TextHighLight);
  xbutton += 60; 
  ...
  openButton = new SimpleButton("OpenTest", xbutton, ybutton, font_CB, 20, LabelColor, TextHighLight); 
  xbutton += 80;
  saveButton = new SimpleButton("SaveTest", xbutton, ybutton, font_CB, 20, LabelColor, TextHighLight);

  xbutton = 655;

  urlButton = new SimpleButton("AxiDraw.com", xbutton, ybutton, font_CB, 20, LabelColor, TextHighLight);

  UIMessage = new SimpleButton("Welcome to test! Hold 'h' key for help!", 
...
void draw() {
  ...
  if (debugMode)
  {
    frame.setTitle("test      " + int(frameRate) + " fps");
  }
  ...
  if (doSerialConnect)
  {
    ...
    {    
      ...
      UIMessage.label = "Welcome to test!  Hold 'h' key for help!";
...
void mousePressed() {
  ...
  } else if ( openButton.isSelected() )  
  {
    ...
    selectInput("Select an test (.rrt) file to open:", "fileSelected");  // Opens file chooser
  }
}

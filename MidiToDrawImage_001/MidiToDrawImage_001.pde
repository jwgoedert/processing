...
void rawMidi(byte[] data) { // You can also use rawMidi(byte[] data, String bus_name)
  ...
  for (int i = 1;i < data.length;i++) {
    ...
    println("terstin");
  // if(data[1] == 70) {
  //    x = map(data[2] & 0xFF, 0, 127, 0, width);
  //    ellipse(x,y, 25,25); 
  //}
  //  else if(data[1] == 74) {
  //    y = map(data[2] & 0xFF, 0, 127, 0, height);
  //    ellipse(x,y, 25,25); 
  //  } else
    //if(data[1] < 70){
      //    y = map(data[1] & 0xFF, 48, 72, height, 0);
      //    x = x + 25;
      //    if(x > width){
      //      x = 0;
      //    }
...
void midiMessage(MidiMessage message) { // You can also use midiMessage(MidiMessage message, long timestamp, String bus_name)
  ...
  for (int i = 1;i < message.getMessage().length;i++) {
    ...
    //if(i == 2){
    //  int res = (message.getMessage()[i] & 0xFF);
    //  stroke(128);
    //  //ellipse(res,res,10,10);
    //};
  }
}
